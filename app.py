import asyncio
from fastapi import FastAPI, File, HTTPException, UploadFile, Request, BackgroundTasks, status
...
async def process_pdf_task(file: UploadFile, background_tasks: BackgroundTasks):
    ...
    task = {
        ...
        "progress": [
            {
                "progress_index": 1,
                "progress_message": "1. File Upload in Progress"
            }
        ],
        "result": None
    }
...
async def extract_pdf(pdf_file_path: UploadFile, task: dict):
    try:
        ...
        task["progress"].append(
            {
                "progress_index": 2,
                "progress_message": "1. File Upload Completed"
            }
        )
        ...
        task["progress"].append(
            {
                "progress_index": 3,
                "progress_message": "2. Document OCR in progress"
            }
        )
        ...
        if os.path.exists(output):
            excel_output_s3_presigned_url = await upload_to_s3_generate_presigned_download_url(output)
        ...
        task["progress"].append(
            {
                "progress_index": 8,
                "progress_message": "4. Excel Output Generated"
            }
        )
...
async def get_process_pdf_task_status(task_id: str):
    try:
        ...
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="An error occured while fetching document extraction status")

...
from services.extractor import (
    get_sorted_data_with_page_numbers,
    process_and_extract_document,
)
from services.openai_request import extract_data_azure_openai
from services.response_formatter import format_os_response
from utils.calculate_confidence_score_and_coordinates_list import (
    update_confidence_score_with_coordinates,
)
from utils.calculate_totals import calculate_totals
from utils.common import create_subfolders, get_filename_without_extension
...
async def process_pdf_document(pdf_path, task):
    ...
    try:
        ...
        task["progress"].append(
            {"progress_index": 4, "progress_message": "2. Document OCR Completed"}
        )
        task["progress"].append(
            {
                "progress_index": 5,
        ...
        extracted_text, azure_ocr_json_response = (
            get_text_from_pdf_with_doc_intelligence(pdf_path)
        )

        json_response = {}

        # get balance sheet
        balance_sheet_user_prompt = get_balance_sheet_user_prompt(
            updated_balance_sheet_prompt
        )
        balance_sheet_openai_response, _ = await extract_data_azure_openai(
            balance_sheet_user_prompt, extracted_text, balance_sheet_user_feedback
        )

        balance_sheet_json_response = json.loads(balance_sheet_openai_response)

        json_response["is_balance_sheet_present"] = balance_sheet_json_response[
            "is_balance_sheet_present"
        ]
        ...
        income_statement_user_prompt = get_income_statement_user_prompt(
            updated_income_statement_prompt
        )
        income_statement_openai_response, _ = await extract_data_azure_openai(
            income_statement_user_prompt, extracted_text, income_statement_user_feedback
        )

        income_statement_json_response = json.loads(income_statement_openai_response)

        json_response["is_income_statement_present"] = income_statement_json_response[
            "is_income_statement_present"
        ]
        json_response["income_statement"] = income_statement_json_response[
            "income_statement"
        ]
        ...
        final_json_response = update_confidence_score_with_coordinates(
            final_json_response, azure_ocr_json_response
        )
        with open(json_response_path, "w") as file:
            file.write(json.dumps(final_json_response, indent=2))

        excel_output_path = json_to_xlsx(
            final_json_response,
        ...
        task["progress"].append(
            {"progress_index": 7, "progress_message": "4. Generating Excel Output"}
        )
        ...
        try:
            ...
            onedrive_folder_graph_url = await mgu.get_drive_path(
                mgu.onedrive_url, token=access_token
            )

            if onedrive_folder_graph_url and os.path.exists(log_path):
                await mgu.upload_to_onedrive(
                    onedrive_folder_graph_url,
                    pdf_filename_without_extension,
                    log_path,
                    token=access_token,
                )
            ...
            if onedrive_folder_graph_url and os.path.exists(ocr_response_text_path):
                await mgu.upload_to_onedrive(
                    onedrive_folder_graph_url,
                    pdf_filename_without_extension,
                    ocr_response_text_path,
                    token=access_token,
                )
                folder_path_list.append(os.path.dirname(ocr_response_text_path))

            if onedrive_folder_graph_url and os.path.exists(
                azure_ocr_json_response_path
            ):
                await mgu.upload_to_onedrive(
                    onedrive_folder_graph_url,
                    pdf_filename_without_extension,

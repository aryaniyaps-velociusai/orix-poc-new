            ...
            for key, excel_label in field_mapping.items():
                if field_label == excel_label and key in data_values:
                    ...
                    if not data_values[key].get("is_total_field", False):
                        ...
                        formatted_comment += "\n".join(
                            [
                                f'{field["field_name"]} : {field["value"]},  Page-{field["page_number"]}'
                                for field in data_values[key]["fields"]
                            ]
                        )
                        row[2].value = formatted_comment
                        row[2].alignment = styles.Alignment(
    ...
    try:
        ...
        if data_json.get("is_balance_sheet_present", "No") == "Yes":
            populate_sheet(
                balance_sheet_sheet,
                {
                    **operating_statement_excel_field_mapping["balance_sheet"][
                        "assets"
                    ],
                    **operating_statement_excel_field_mapping["balance_sheet"][
                        "liabilities"
                ...
                {
                    **data_json["balance_sheet"]["assets"],
                    **data_json["balance_sheet"]["liabilities"],
                },
            )
        if data_json.get("is_income_statement_present", "No") == "Yes":
            populate_sheet(
                ...
                {
                    **operating_statement_excel_field_mapping["income_statement"][
                        "revenue_income"
                    ],
                    **operating_statement_excel_field_mapping["income_statement"][
                        "expenses"
                ...
                {
                    **data_json["income_statement"]["revenue_income"],
                    **data_json["income_statement"]["expenses"],
                },
            )
...
def document_json_to_xlsx(
    data_json, template_path, pdf_filename_without_extension, timestamp
):
    return json_to_xlsx(
        data_json, template_path, pdf_filename_without_extension, timestamp
    )

...
def convert_polygon_list(polygon):
    ...
    for i in range(0, len(polygon), 2):
        point = {"x": float(polygon[i]), "y": float(polygon[i + 1])}
...
def match_respective_words(words, value, azure_ocr_page, word_index, page_number):
    ...
    try:
        if len(words) == 1 and check_value_type(value):
            ...
            if (
                not re.match(r"^\d+\.?\d{0,2}$", cleaned_azure_ocr_word)
                or cleaned_azure_ocr_word == ""
            ):
                return False
            else:
                try:
                    return (
                        f"{float(cleaned_azure_ocr_word):.2f}"
                        == f"{float(value):.2f}".replace("-", "")
                    )
        ...
        if len(words) > 0:
            ...
            if (
                fuzz.ratio(
                    azure_ocr_page["words"][word_index]["content"].lower(),
                    words[0].lower(),
                )
                > 95
            ):
                ...
                for i in range(len(words)):
                    if word_index + i >= len(azure_ocr_page["words"]):
                        return False
                    output_str_lst.append(
                        azure_ocr_page["words"][word_index + i]["content"].lower()
                    )
...
def calculate_confidence_score_and_net_polygon_value(
    words, value, azure_ocr_output, page_number
):
    total_confidence = 0
    count = 0
    net_polygon_value = []
    ...
    try:
        if page_number <= len(azure_ocr_output["pages"]):
            ...
            for word_index in range(len(azure_ocr_page["words"]) - 1):
                if match_respective_words(
                    words, value, azure_ocr_page, word_index, page_number
                ):
                    total_confidence = 0
                    count = 0
                    for index in range(len(words)):
                        total_confidence += azure_ocr_page["words"][word_index + index][
                            "confidence"
                        ]
                        coordinates_list = convert_polygon_list(
                            azure_ocr_page["words"][word_index + index]["polygon"]
                        )
        ...
        print(
            "error occured in",
            calculate_confidence_score_and_net_polygon_value.__name__,
        )
        raise e


def update_categories_with_confidence_score_and_coordinates(
    document, azure_ocr_output, category, subcategory
):
    try:
            ...
            if len(value["fields"]) > 0:
                ...
                for field_index in range(len(value["fields"])):
                    ...
                    if len(field_value) > 0 and len(field_name) > 0:
                        ...
                        (
                            field_name_confidence_score,
                            field_name_net_polygon_value,
                            page_width,
                            page_height,
                        ) = calculate_confidence_score_and_net_polygon_value(
                            field_name_words,
                            field_name,
                            azure_ocr_output,
                            value["fields"][field_index]["page_number"],
                        )
                        (
                            field_value_confidence_score,
                            field_value_net_polygon_value,
                            page_width,
                            page_height,
                        ) = calculate_confidence_score_and_net_polygon_value(
                            field_value_words,
                        ...
                        document[category][subcategory][key]["fields"][field_index] = {
                            ...
                            "field_name_confidence_score": round(
                                field_name_confidence_score, 4
                            ),
                            "field_value_confidence_score": round(
                                field_value_confidence_score, 4
                            ),
                        ...
                        fields_confidence_score_list.append(
                            field_value_confidence_score
                        )
                ...
                document[category][subcategory][key]["confidence_score"] = sum(
                    fields_confidence_score_list
                ) / len(fields_confidence_score_list)
                document[category][subcategory][key]["coordinates_list"] = (
                    fields_coordinate_list
                )
        return document
    except Exception as e:
        print(
            f"Error occured in : {update_categories_with_confidence_score_and_coordinates.__name__}",
            e,
        )
        raise e
...
def update_confidence_score_with_coordinates(document, azure_ocr_output):
    try:
        if document.get("is_balance_sheet_present", "No") == "Yes":
            document = update_categories_with_confidence_score_and_coordinates(
                document, azure_ocr_output, "balance_sheet", "assets"
            )
            document = update_categories_with_confidence_score_and_coordinates(
                document, azure_ocr_output, "balance_sheet", "liabilities"
            )

        if document.get("is_income_statement_present", "No") == "Yes":
            document = update_categories_with_confidence_score_and_coordinates(
                document, azure_ocr_output, "income_statement", "revenue_income"
            )
            document = update_categories_with_confidence_score_and_coordinates(
                document, azure_ocr_output, "income_statement", "expenses"
            )
...
if __name__ == "__main__":
    with open(
        "./azure_ocr_json_response_folder/azure_ocr_json_response_107311---DPO---Operating-Statements---9-30-2023---THE-TERRACE-OF-HAMMOND-PHASE-I_1725880473.json"
    ) as file:
        azure_rsponse = json.load(file)

    with open(
        "./formatted_json_response_folder/formatted_json_response_107311---DPO---Operating-Statements---9-30-2023---THE-TERRACE-OF-HAMMOND-PHASE-I_1725880473.json"
    ) as file:
